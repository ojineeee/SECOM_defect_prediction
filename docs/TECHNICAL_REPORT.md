...
라벨을 사실상 쓰지 않는 One-class 이상탐지는 지도학습 대비 Recall이 약 1/8 수준입니다
(`10_anomaly_vs_supervised.png`). PR-AUC(0.055~0.057)는 이 테스트셋의 불량 비율(약 6.7%)과
큰 차이가 없어, 사실상 무작위 순위와 구분하기 어려운 수준입니다. **이 데이터셋에서는 소량이라도
라벨이 있다면 지도학습을 쓰는 것이 명확히 유리**하며, One-class 이상탐지는 "불량 라벨이 전혀
없는 신규 공정 초기" 같은 상황에서만 대안으로 고려할 만합니다.
...
Recall이 정확히 0(21건 중 0건)이라 부트스트랩(고정된 예측 벡터를 재표본추출)으로는 CI를 구성할
수 없다. 대신 "실제 불량 17건 중 몇 건을 맞혔는가"를 이항 비율(k=0, n=17)로 보고 Clopper-Pearson
정확 신뢰구간을 계산하면 **[0.0, 0.1951]** 이다. 무작위 분할의 Recall CI(0.1667~0.5882)와 거의
겹치지 않는다는 점에서, 이 성능 차이는 우연한 표본 변동으로 설명하기 어렵다.
