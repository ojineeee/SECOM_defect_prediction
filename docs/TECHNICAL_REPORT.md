...
| 시간순 단일 분할 | 0.000 (0.000~0.000) | 0.084 (0.045~0.171) |
...
| Fold | 평가 구간 | n_test_fail | Recall | PR-AUC |
|---|---|---|---|---|
| 1 | 08-19 ~ 09-01 | 21 | 0.619 | 0.055 |
| 2 | 09-01 ~ 09-20 | 11 | 0.182 | 0.060 |
| 3 | 09-20 ~ 10-02 | 11 | 0.000 | 0.036 |
| 4 | 10-02 ~ 10-17 | 17 | 0.000 | 0.083 |

Recall이 0.619 → 0.182 → 0.000 → 0.000으로 단조 감소하는 패턴을 보임 (`18_walk_forward_recall.png`).
이는 성능 붕괴가 갑작스러운 단절이 아니라 **점진적 개념 드리프트**(concept drift)의 결과이며,
초기 구간에서는 랜덤 분할보다도 높은 Recall이 나온다는 점에서 모델 자체가 무의미한 것이 아니라
"재학습 없이 오래 방치했을 때" 성능이 붕괴한다는 훨씬 구체적인 그림을 제공한다.
