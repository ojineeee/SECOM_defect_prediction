...
- 불량 샘플이 104개뿐이라 test set(랜덤 분할 21개 / 시간순 분할 17개)의 결과가 몇 건의 예측에 따라 크게 흔들릴 수 있어, 성능 수치는 신뢰구간 없이 point estimate로만 해석해야 합니다.
- SMOTE는 합성 샘플을 생성하므로 실제 물리적으로 존재하지 않는 센서 조합을 만들어낼 수 있다는 한계가 있습니다.
- 시간순 분할 검증 결과 Recall이 0까지 떨어지는 것을 확인했습니다. 이 모델을 실제로 배포하려면 최소한 주기적 재학습, concept drift 감지, 혹은 시간에 강건한 피처 설계가 추가로 필요합니다.
- 이번 실험은 단일 분할 기준이며, 여러 시드/분할 기준으로 반복 실험하면 결과가 달라질 수 있습니다.
...
│   └── chronological_validation.py  # 시간순 분할 재검증
├── results/
│   ├── figures/          # 01~15 시각화 결과
│   └── *.json, *.csv     # 수치 결과
├── requirements.txt
└── run_all.sh
```
