...
4단계. 결과를 정직하게 해석한다         →  잘 된 점과 한계를 함께 정리
```
...
- 시간이 지날수록 불량률이 줄어드는 드리프트가 있는데도, 평가는 여전히 무작위 분할(랜덤 20%)을 사용했습니다. 실제 운영을 가정하면 "앞 80%로 학습, 뒤 20%로 평가"처럼 시간순으로 나누는 것이 더 현실적인 검증이며, 이번 프로젝트에서는 다루지 않았습니다.
